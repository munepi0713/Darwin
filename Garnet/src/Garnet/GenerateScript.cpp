...
void Garnet::generateScript(Garnet::PPEGraph& pg, std::ostream& outs, const ConfigStore& conf, TraceStore& trace)
{
	StopWatch watch[3];
	...
	switch ( conf.read<unsigned>("/generateScript/Generate Script/TopologicalSortMethod") ) {
	case TOPOSORT_BGL :
		topological_sort(1, pg, std::back_inserter(nodes)); // BGL-standard topo sort
		break;
	case TOPOSORT_DEFAULT :
